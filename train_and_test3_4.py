...
from utils import get_mean_std,create_net,ReadStaPoints_test
...
def train_run(val_list,train_list,model,
    ...
    # #拿最佳模型进行预测
    # if predict_save_dir is not None and model_save_dir is not None:
    #     model_path=os.path.join(model_save_dir,f'{cfg.Y_NAME}_{bestepoch}.pth')
    #     if validate:test_list=val_list
    #     else:test_list=os.listdir(os.path.join(cfg.TEST_DIR,"x"))
    #     test_predict(cfg.TEST_STATION_IDR,predict_save_dir,test_list,vals_idx=vals_idx,model_path=model_path,validate=validate)  
...
def train_and_test(vals_idx=None,device=cfg.DEVICE,
    ...
    train_run(val_list,train_list,model,
                ...
                vals_idx=vals_idx,iter_show=iter_show,epoch_show=epoch_show,
                device=device)
    print('训练完毕！')
...
def K_fold_val(K=5,
    ...
    for i in range(2,4):
        ...
        train_run(val_list,train_list,model,
                ...
                iter_show=iter_show,epoch_show=epoch_show,
                device=device)
    print('K折交叉验证完毕!')
